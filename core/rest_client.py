...
from core.mock_data import get_api_mock


class RestClient():

    def __init__(self, api_root_url, use_mock=False):
    ...
    def request(self, url, method, data=None, json=None, **kwargs):
        ...
        # 修复原 bug：files / cookies 误用了 params，改为各自取值
        files = dict(**kwargs).get("files")
        cookies = dict(**kwargs).get("cookies")
        self.request_log(url, method, data, json, params, headers, files, cookies)

        # 离线学习模式：不发真实请求，交给契约 mock 路由
        if self.use_mock:
            # 优先取 json 体（用例多传 json），否则取 data
            request_body = json or data or {}
            if isinstance(request_body, str):
                try:
                    request_body = complexjson.loads(request_body)
                except Exception:
                    request_body = {}
            status_code, body = get_api_mock(method, url, request_body)
            mock_res = requests.Response()
            mock_res.status_code = status_code
            mock_res._content = complexjson.dumps(body).encode("utf-8")
            mock_res.url = url
            return mock_res
